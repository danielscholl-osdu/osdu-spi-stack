...
- Multi-partition is verified GA: `Config.data_partitions` carries a Pydantic validator (`src/spi/config.py`) that rejects names which would silently truncate or collide at Azure-resource-naming time, `spi info` and `spi status` enumerate per-partition resources and bootstrap Job health, and `infra/params/multi.bicepparam` ships a two-partition example covered by the existing `test_bicepparam_files_compile` smoke test.
