...
| Federated subject (PR builds) | `repo:danielscholl-osdu/osdu-spi-stack:pull_request` |
| Federated subject (main builds) | `repo:danielscholl-osdu/osdu-spi-stack:ref:refs/heads/main` |
| Federated subject (smoke env) | `repo:danielscholl-osdu/osdu-spi-stack:environment:azure-smoke` |
...
for SUBJECT in \
  "repo:danielscholl-osdu/osdu-spi-stack:pull_request" \
  "repo:danielscholl-osdu/osdu-spi-stack:ref:refs/heads/main" \
  "repo:danielscholl-osdu/osdu-spi-stack:environment:azure-smoke"; do
  SHORT_NAME=$(echo "$SUBJECT" | sed 's|repo:danielscholl-osdu/osdu-spi-stack:||; s|[:/]|-|g')
...
gh secret set AZURE_CLIENT_ID --body "$APP_ID" --repo danielscholl-osdu/osdu-spi-stack
gh secret set AZURE_TENANT_ID --body "<TENANT_ID>" --repo danielscholl-osdu/osdu-spi-stack
gh secret set AZURE_SUBSCRIPTION_ID --body "<SUBSCRIPTION_ID>" --repo danielscholl-osdu/osdu-spi-stack
...
gh api -X PUT "repos/danielscholl-osdu/osdu-spi-stack/environments/azure-smoke" \
  --input - <<EOF
{
  "wait_timer": 0,
...
```bash
gh api -X PUT repos/danielscholl-osdu/osdu-spi-stack/branches/main/protection \
...
```bash
gh api repos/danielscholl-osdu/osdu-spi-stack/branches/main/protection \
  --jq '{
