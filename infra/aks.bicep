// Stage A SPIKE: AKS Automatic + managed Istio via Azure Verified Modules.
//
// This template is a throwaway exploration to decide whether the AVM
// managed-cluster module can express the full AKS Automatic + Istio
...
// created in infra/main.bicep after this template outputs the OIDC issuer
// URL for federated credentials.
//
// Known AVM gaps (v0.13.0) that remain imperative post-deploy:
//   1. safeguardsProfile is not exposed (use `az aks safeguards update`
//      or fallback `az aks update --safeguards-level Warning`)
...
module aksCluster 'br/public:avm/res/container-service/managed-cluster:0.13.0' = {
  ...
  params: {
    ...
    // user-assigned identity created elsewhere.
    managedIdentities: {
      systemAssigned: true
    }
